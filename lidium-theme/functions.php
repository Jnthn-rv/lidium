...
function lidium_theme_scripts() {
    ...
    wp_enqueue_style('lidium-style', get_stylesheet_uri());
    wp_enqueue_script('lidium-custom-js', get_template_directory_uri() . '/js/custom.js', array(), '1.0', true);
}
add_action('wp_enqueue_scripts', 'lidium_theme_scripts');

// Registrar menús de navegación
function lidium_register_nav_menu(){
...
add_action( 'after_setup_theme', 'lidium_register_nav_menu' );
...
class Walker_Nav_Primary extends Walker_Nav_Menu {
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) { $output .= '<li><a class="nav-link" href="' . esc_url($item->url) . '">' . esc_html($item->title) . '</a>'; }
...
class Walker_Nav_Mobile extends Walker_Nav_Menu {
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) { $output .= '<li><a class="block py-2 text-gray-700 hover:text-[#7732a8]" href="' . esc_url($item->url) . '">' . esc_html($item->title) . '</a>'; }
    function end_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) { $output .= '</li>'; }
    function start_lvl(&$output, $depth=0, $args=null) {}
    function end_lvl(&$output, $depth=0, $args=null) {}
}
...
function lidium_create_custom_post_types() {
    // Testimonios
    $labels_testimonial = array( 'name' => _x( 'Testimonios', 'Post Type General Name', 'lidium-theme' ), 'singular_name' => _x( 'Testimonio', 'Post Type Singular Name', 'lidium-theme' ), 'menu_name' => __( 'Testimonios', 'lidium-theme' ), 'name_admin_bar' => __( 'Testimonio', 'lidium-theme' ), 'archives' => __( 'Archivo de Testimonios', 'lidium-theme' ), 'add_new_item' => __( 'Añadir Nuevo Testimonio', 'lidium-theme' ), 'add_new' => __( 'Añadir Nuevo', 'lidium-theme' ),);
    $args_testimonial = array('label' => __( 'Testimonio', 'lidium-theme' ), 'description' => __( 'Testimonios de clientes', 'lidium-theme' ), 'labels' => $labels_testimonial, 'supports' => array( 'title', 'thumbnail' ), 'hierarchical' => false, 'public' => true, 'show_ui' => true, 'show_in_menu' => true, 'menu_position' => 5, 'menu_icon' => 'dashicons-format-quote', 'show_in_admin_bar' => true, 'show_in_nav_menus' => true, 'can_export' => true, 'has_archive' => false, 'exclude_from_search' => true, 'publicly_queryable' => true, 'capability_type' => 'post',);
    register_post_type( 'testimonial', $args_testimonial );

    // Proyectos
    $labels_project = array('name' => _x( 'Proyectos', 'Post Type General Name', 'lidium-theme' ), 'singular_name' => _x( 'Proyecto', 'Post Type Singular Name', 'lidium-theme' ), 'menu_name' => __( 'Proyectos', 'lidium-theme' ), 'name_admin_bar' => __( 'Proyecto', 'lidium-theme' ), 'archives' => __( 'Archivo de Proyectos', 'lidium-theme' ), 'add_new_item' => __( 'Añadir Nuevo Proyecto', 'lidium-theme' ), 'add_new' => __( 'Añadir Nuevo', 'lidium-theme' ),);
    $args_project = array('label' => __( 'Proyecto', 'lidium-theme' ), 'description' => __( 'Proyectos del portafolio', 'lidium-theme' ), 'labels' => $labels_project, 'supports' => array( 'title', 'editor', 'thumbnail' ), 'hierarchical' => false, 'public' => true, 'show_ui' => true, 'show_in_menu' => true, 'menu_position' => 6, 'menu_icon' => 'dashicons-portfolio', 'show_in_admin_bar' => true, 'show_in_nav_menus' => true, 'can_export' => true, 'has_archive' => true, 'exclude_from_search' => false, 'publicly_queryable' => true, 'capability_type' => 'post',);
    register_post_type( 'project', $args_project );
}
add_action( 'init', 'lidium_create_custom_post_types', 0 );

// --- Modify Main Blog Query to Exclude Featured Post --- //
...
// --- Registrar Página de Opciones del Tema con ACF --- //
...
// --- Registrar Campos Personalizados (ACF) con PHP (HOME SETTINGS) --- //
